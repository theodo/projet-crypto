...
class KrakenChartLTC extends Component {
  ...
  CallKraken(startDate,period) {
      axios(`https://api.kraken.com/0/public/OHLC?pair=LTCUSD&interval=${period}&since=${startDate}`)
      .then((response) => {

        const Data = response.data.result;

        const chartData = {
          labels: Data.XLTCZUSD.map(k => timeConverter(k[0])),
          datasets: [
            {
              label: 'Price LTC/USD',
                  fill: false,
